...
int main(int argc, char *argv[]) {
    ...
    int min_temp = get_min_temp_historic(array);
    printf("La menor temperatura minima historica es: %d\n", min_temp);

    int max_temp[YEARS];
    get_max_temp(array, max_temp);
    printf("Las mayores temperaturas maximas historicas son: \n");
    for(unsigned int i = 0; i < YEARS; i++){
        printf("Anio %u: %d\n", (i + 1980), max_temp[i]);
    }

    month_t precips_maxs[YEARS];
    most_rainfalls(array, precips_maxs);
